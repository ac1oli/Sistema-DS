...
<head>
    ...
    <title>ESTOQUE - Cabeça De Impressão</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Cabeça De Impressão</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Cabeça D. Imp.</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php
    
        // Buscar apenas os equipamentos da categoria 'Cabeça De Impressão'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'ativo' AND local = 'PE' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_cabecas FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'ativo' AND local = 'PE' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Cabeças:  $total_cabecas </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'ativo' AND local = 'PE'";
