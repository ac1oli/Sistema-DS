...
<head>
    ...
    <title>Saida - Cabeça De Impressão</title>
</head>
<body>

    <nav>
        <h1>Cabeça De Impressão - Que sairam do estoque</h1>
    ...
    <?php
        ...
        if(!empty($_GET['pesquisa'])){
            ...
            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'inativo' AND local = 'PE' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_cabecas FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'inativo' AND local = 'PE' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'inativo' AND local = 'PE'";

            $sql_count = "SELECT COUNT(*) AS total_cabecas FROM equipamento WHERE categoria = 'Cabeça De Impressão' AND status = 'inativo' AND local = 'PE'";
            $result_count = mysqli_query($conexao, $sql_count);
