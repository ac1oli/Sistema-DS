...
<head>
    ...
    <title>Saida - Monitor Touch</title>
</head>
<body>

    <nav>
        <h1>Monitor Touch Que sairam do estoque</h1>
    ...
    <?php
        ...
        if(!empty($_GET['pesquisa'])){
            ...
            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'inativo' AND local = 'RJ-Centro' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_monitortouch FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'inativo' AND local = 'RJ-Centro' AND
            ...
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'inativo' AND local = 'RJ-Centro'";

            $sql_count = "SELECT COUNT(*) AS total_monitortouch FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'inativo' AND local = 'RJ-Centro'";
            $result_count = mysqli_query($conexao, $sql_count);
