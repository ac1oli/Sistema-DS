...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'Impressora Térmica'; // fixa a categoria
    ...
    if (!$id && !isset($_POST["editar"]) && !isset($_POST["excluir"])) {
        ...
        $sql = "INSERT INTO equipamento 
            (categoria, local, marca, modelo, serial, patrimonio, obs, data_entrada, colaborador) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssss", $categoria, $local, $marca, $modelo, $serial, $patrimonio, $obs, $data_entrada, $nomeColaborador);
