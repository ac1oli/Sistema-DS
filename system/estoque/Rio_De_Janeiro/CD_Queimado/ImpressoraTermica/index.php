...
<head>
    ...
    <title>ESTOQUE - Impressora Termica</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Impressora Termica</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Impressora Termica</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

        // Buscar apenas os equipamentos da categoria 'impressoraT'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Impressora Térmica' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_impressoraT FROM equipamento WHERE categoria = 'Impressora Térmica' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Impressora Termica:  $total_impressoraT </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Impressora Térmica' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
