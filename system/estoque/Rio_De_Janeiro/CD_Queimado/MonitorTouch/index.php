...
<head>
    ...
    <title>ESTOQUE - Monitor Touch</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Monitor touch</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Monitor Touch</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>


    <?php
    // Buscar apenas os equipamentos da categoria 'monitortouch'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_monitortouch FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de monitortouch:  $total_monitortouch </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Monitor Touch' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
            ...
            if ($result_count) {
                $row_count = mysqli_fetch_assoc($result_count);
                $total_monitortouch = $row_count['total_monitortouch'];
                echo "<h3> Total de Monitor Touch:  $total_monitortouch </h3>";
            }
        }
