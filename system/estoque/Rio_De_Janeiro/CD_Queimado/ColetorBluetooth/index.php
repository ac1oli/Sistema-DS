...
<head>
    ...
    <title>ESTOQUE - Coletor Bluetooth</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Coletor Bluetooth</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Coletor Bluetooth</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

         // Buscar apenas os equipamentos da categoria 'coletorB'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Coletor Bluetooth' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_coletorB FROM equipamento WHERE categoria = 'Coletor Bluetooth' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Coletor Bluetooth:  $total_coletorB </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Coletor Bluetooth' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
