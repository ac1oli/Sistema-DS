...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'notebook'; // fixa a categoria
    ...
    $processador = $_POST["processador"] ?? NULL;
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            (categoria, local, marca, modelo, serial, patrimonio sistema_operacional, processador, memoria, armazenamento, obs, data_entrada, colaborador) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssssssss", $categoria, $local, $marca, $modelo, $serial, $patrimonio, $sistema_operacional, $processador, $memoria, $armazenamento, $obs, $data_entrada, $nomeColaborador);
