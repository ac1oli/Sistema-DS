...
<head>
    ...
    <title>Saida - Teclado</title>
</head>
<body>

    <nav>
        <h1>Teclado Que sairam do estoque</h1>
    ...
    <?php
        ...
        // Buscar apenas os equipamentos da categoria 'teclador'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'teclador' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_teclador FROM equipamento WHERE categoria = 'teclador' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                $total_teclador = $row_count['total_teclador'];
                echo "<h3> Total de Teclador:  $total_teclador </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'teclador' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_teclador FROM equipamento WHERE categoria = 'teclador' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
