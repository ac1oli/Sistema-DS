...
<head>
    ...
    <title>ESTOQUE - Monitor</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Monitor</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Monitor</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

        // Buscar apenas os equipamentos da categoria 'monitor'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'monitor' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_monitor FROM equipamento WHERE categoria = 'monitor' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de monitor:  $total_monitor </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'monitor' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
