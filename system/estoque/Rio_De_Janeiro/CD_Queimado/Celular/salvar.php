...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'celular'; // fixa a categoria
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            ( categoria, local, marca, modelo, serial, imei_equip, patrimonio, memoria, armazenamento, obs, data_entrada, colaborador) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssssssss", $categoria, $local, $marca, $modelo, $serial, $imei_equip, $patrimonio, $memoria, $armazenamento, $obs, $data_entrada, $nomeColaborador);
