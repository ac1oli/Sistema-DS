...
<head>
    ...
    <title>ESTOQUE - Relogio De Ponto</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Relogio De Ponto</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Relogio De Ponto</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

        // Buscar apenas os equipamentos da categoria 'relogioP'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Relogio De Ponto' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_relogioP FROM equipamento WHERE categoria = 'Relogio De Ponto' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Relogio de Ponto:  $total_relogioP </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Relogio De Ponto' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
