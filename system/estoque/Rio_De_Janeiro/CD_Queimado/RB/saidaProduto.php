...
<head>
    ...
    <title>Saida - RB</title>
</head>
<body>

    <nav>
        <h1>Router Board Que sairam do estoque</h1>
    ...
    <h3>
        ...
        <p><a href="indexrb.php">Produtos no estoque</a></p>
    </h3>
    ...
    <?php
        ...
        if(!empty($_GET['pesquisa'])){
            ...
            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Router Board' AND status = 'inativo'AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_rb FROM equipamento WHERE categoria = 'Router Board' AND status = 'inativo'AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de RB:  $total_rb </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Router Board' AND status = 'inativo'AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_rb FROM equipamento WHERE categoria = 'Router Board' AND status = 'inativo'AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
