...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'Chip de Celular'; // fixa a categoria
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            (categoria, local, marca, modelo, imei_equip, linha, obs, data_entrada, colaborador) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssss", $categoria, $local, $marca, $modelo, $imei_equip, $linha, $obs, $data_entrada, $nomeColaborador);
