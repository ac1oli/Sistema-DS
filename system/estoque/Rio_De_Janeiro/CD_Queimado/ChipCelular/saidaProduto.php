...
<head>
    ...
    <title>Saida - chip</title>
</head>
<body>

    <nav>
        <h1>Chip que sairam do estoque</h1>
    ...
    <?php
        ...
        if(!empty($_GET['pesquisa'])){
            ...
            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Chip de Celular' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_chip FROM equipamento WHERE categoria = 'Chip de Celular' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Chip de Celular' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_chip FROM equipamento WHERE categoria = 'Chip de Celular' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
