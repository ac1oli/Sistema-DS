...
<head>
    ...
    <title>Saida - Mouse</title>
</head>
<body>

    <nav>
        <h1>Mouse Que sairam do estoque</h1>
    ...
    <?php
        // Buscar apenas os equipamentos da categoria 'mouse'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'mouse' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_mouse FROM equipamento WHERE categoria = 'mouse' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Mouse:  $total_mouse </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'mouse' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_mouse FROM equipamento WHERE categoria = 'mouse' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
