...
<head>
    ...
    <title>ESTOQUE - Impressora</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Impressora</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Impressora</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

         // Buscar apenas os equipamentos da categoria 'impressora'
        if(!empty($_GET['RJ-CD-Queimadosquisa'])){

            $pesq = $_GET['RJ-CD-Queimadosquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Impressora' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_impressora FROM equipamento WHERE categoria = 'Impressora' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Impressora:  $total_impressora </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Impressora' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
...
<script>
    var pesquisa = document.getElementById('RJ-CD-Queimadosquisar');
