...
<head>
    ...
    <title>ESTOQUE - Fonte Dell</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Fonte Dell</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Fonte Dell</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

        // Buscar apenas os equipamentos da categoria 'fonteDell'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Fonte DELL' AND status = 'ativo'AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_fonteDell FROM equipamento WHERE categoria = 'Fonte DELL' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Fonte DELL' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_fonteDell FROM equipamento WHERE categoria = 'Fonte DELL' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
