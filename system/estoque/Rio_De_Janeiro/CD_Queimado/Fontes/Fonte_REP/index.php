...
<head>
    ...
    <title>ESTOQUE - Fonte REP</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Fonte REP</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Fonte Rep</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php

        // Buscar apenas os equipamentos da categoria 'fonteRep'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Fonte REP' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_fonteRep FROM equipamento WHERE categoria = 'Fonte REP' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de FonteREP:  $total_fonteRep </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'Fonte REP' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_fonteRep FROM equipamento WHERE categoria = 'Fonte REP' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);

            if ($result_count) {
                $row_count = mysqli_fetch_assoc($result_count);
                $total_fonteRep = $row_count['total_fonteRep'];
                echo "<h3> Total de Fonte REP:  $total_fonteRep </h3>";
            }
        }
