...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'HD'; // fixa a categoria
    $local = 'RJ-CD-Queimado';
    ...
    if (!$id && !isset($_POST["editar"]) && !isset($_POST["excluir"])) {
        ...
        $sistema_operacional = $_POST["sistema_operacional"] ?? NULL;
        $obs = $_POST["obs"] ?? NULL;
        ...
        $sql = "INSERT INTO equipamento 
            (categoria, local,  marca, modelo, serial, armazenamento, sistema_operacional, obs, data_entrada, colaborador) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?,?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssssss", $categoria, $local, $marca, $modelo, $serial, $armazenamento, $sistema_operacional, $obs, $data_entrada, $nomeColaborador);
    ...
    if (isset($_POST["dar_saida"]) && $id) {
        ...
        if ($stmt_select = mysqli_prepare($conexao, $sql_select)) {
            ...
            SET status = 'inativo', data_saida = ?, motivo_saida = ? 
            WHERE id_equip = ? AND categoria = ?";
    
        if ($stmt_update = mysqli_prepare($conexao, $sql_update)) {
            mysqli_stmt_bind_param($stmt_update, "ssis", $data_saida, $motivo_saida, $id, $categoria);
    ...
    if (isset($_POST["voltar_estoque"]) && $id) {
        ...
        if ($stmt_select = mysqli_prepare($conexao, $sql_select)) {
            ...
            $sql_update = "UPDATE equipamento 
            SET status = 'ativo', data_saida = ?, motivo_saida = ? 
            WHERE id_equip = ? AND categoria = ?";
