...
<head>
    ...
    <title>ESTOQUE - Switch</title>
</head>
<body>

    <nav>
        <h1>ESTOQUE - Switch</h1>
    ...
    <h3>
        ...
        <a href="adicionar.php">Adicionar Switch</a>
        <a href="saidaProduto.php">Saida - Estoque</a>
    </h3>

    <?php
    
        // Buscar apenas os equipamentos da categoria 'switch'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'switch' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_switch FROM equipamento WHERE categoria = 'switch' AND status = 'ativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de Switch:  $total_switch </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'switch' AND status = 'ativo' AND local = 'RJ-CD-Queimado'";
