...
<head>
    ...
    <title>Saida - vivobox</title>
</head>
<body>

    <nav>
        <h1>vivobox Que sairam do estoque</h1>
...
<?php
        ...
        if(!empty($_GET['pesquisa'])){
            ...
            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'vivobox' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_vivobox FROM equipamento WHERE categoria = 'vivobox' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de vivobox:  $total_vivobox </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'vivobox' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_vivobox FROM equipamento WHERE categoria = 'vivobox' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
