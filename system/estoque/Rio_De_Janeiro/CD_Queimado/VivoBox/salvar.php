...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $id = $_POST["id"] ?? NULL;
    $categoria = 'vivobox'; // fixa a categoria
    ...
    $imei_chip = $_POST["imei_chip"] ?? NULL;
    $login_adm = $_POST["login_adm"] ?? NULL;
    $senha_adm = $_POST["senha_adm"] ?? NULL;
    $numero_linha = $_POST["numero_linha"] ?? NULL;
    $obs = $_POST["obs"] ?? NULL;
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            ( categoria, local, marca, modelo, serial, ssid, senha, imei_equip, imei_chip, login_adm, senha_adm, numero_linha, obs, data_entrada, colaborador) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssssssssss", $categoria, $local, $marca, $modelo, $serial, $ssid, $senha, $imei_equip, $imei_chip, $login_adm, $senha_adm, $numero_linha, $obs, $data_entrada, $nomeColaborador);
