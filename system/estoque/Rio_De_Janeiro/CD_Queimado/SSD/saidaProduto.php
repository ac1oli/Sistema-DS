...
<head>
    ...
    <title>Saida - SSD</title>
</head>
<body>

    <nav>
        <h1>SSD Que sairam do estoque</h1>
    </nav>
   ...
   <?php
         ...
         // Buscar apenas os equipamentos da categoria 'ssd'
        if(!empty($_GET['pesquisa'])){

            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'ssd' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_ssd FROM equipamento WHERE categoria = 'ssd' AND status = 'inativo' AND local = 'RJ-CD-Queimado' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de SSD:  $total_ssd </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'ssd' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";

            $sql_count = "SELECT COUNT(*) AS total_ssd FROM equipamento WHERE categoria = 'ssd' AND status = 'inativo' AND local = 'RJ-CD-Queimado'";
            $result_count = mysqli_query($conexao, $sql_count);
