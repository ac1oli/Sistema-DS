...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'relogioP'; // fixa a categoria
    ...
    if (!$id && !isset($_POST["editar"]) && !isset($_POST["excluir"])) {
        ...
        $sql = "INSERT INTO equipamento 
            (categoria, marca, modelo, serial, patrimonio, lacre, obs, data_entrada) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssss", $categoria, $marca, $modelo, $serial, $patrimonio, $lacre, $obs, $data_entrada);
