...
<body>
    ...
    <?php
        ...
        if(!empty($_GET['pesquisa'])){
            ...
            $pesq = $_GET['pesquisa'];
            $sql = "SELECT * FROM equipamento WHERE categoria = 'raspbarry' AND status = 'ativo' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";

            $sql_count = "SELECT COUNT(*) AS total_raspbarry FROM equipamento WHERE categoria = 'raspbarry' AND status = 'ativo' AND
            (marca LIKE '%$pesq%' or modelo LIKE'%$pesq%' or serial LIKE '%$pesq%' or data_entrada LIKE'%$pesq%' or patrimonio LIKE '%$pesq%')";
            ...
            if ($result_count) {
                ...
                echo "<h3> Total de RaspBarry:  $total_raspbarry </h3>";
            }

        }else{
            $sql = "SELECT * FROM equipamento WHERE categoria = 'raspbarry' AND status = 'ativo'";

            $sql_count = "SELECT COUNT(*) AS total_raspbarry FROM equipamento WHERE categoria = 'raspbarry' AND status = 'ativo'";
            $result_count = mysqli_query($conexao, $sql_count);
