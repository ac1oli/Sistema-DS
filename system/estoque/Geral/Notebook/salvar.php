...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            (categoria, marca, modelo, serial, patrimonio sistema_operacional, processador, memoria, armazenamento, obs, data_entrada) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssssss", $categoria, $marca, $modelo, $serial, $patrimonio, $sistema_operacional, $processador, $memoria, $armazenamento, $obs, $data_entrada);
