...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST["id"] ?? NULL;
    $categoria = 'leitor de barra'; // fixa a categoria

    // INSERIR
    if (!$id && !isset($_POST["dar_saida"]) && !isset($_POST["excluir"])) {
        ...
        $sql = "INSERT INTO equipamento 
            (categoria, marca, modelo, serial, obs, data_entrada) 
            VALUES (?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssss", $categoria, $marca, $modelo, $serial, $obs, $data_entrada);
