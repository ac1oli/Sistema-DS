...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            ( categoria, marca, modelo, serial, imei_equip, patrimonio, memoria, armazenamento, obs, data_entrada) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssssss", $categoria, $marca, $modelo, $serial, $imei_equip, $patrimonio, $memoria, $armazenamento, $obs, $data_entrada);
