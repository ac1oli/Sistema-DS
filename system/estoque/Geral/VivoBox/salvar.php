...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$id) {
        $sql = "INSERT INTO equipamento 
            ( categoria, marca, modelo, serial, ssid, senha, imei_equip, imei_chip, login_adm, senha_adm, numero_linha, obs, data_entrada) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conexao, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssssssss", $categoria, $marca, $modelo, $serial, $ssid, $senha, $imei_equip, $imei_chip, $login_adm, $senha_adm, $numero_linha, $obs, $data_entrada);
